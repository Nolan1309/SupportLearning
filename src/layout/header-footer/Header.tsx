  ...
  useEffect(() => {
    ...
    const fetchCategories = async () => {
      ...
      try {
        ...
        setLevel1Categories(level1Response.data);

        const level2Response = await axios.get<Category[]>(
          GET_USER_CATEGORY_LEVEL_2
        );
        setLevel2Categories(level2Response.data);

        const level3Response = await axios.get<Category[]>(
          GET_USER_CATEGORY_LEVEL_3
        );
        setLevel3Categories(level3Response.data);
        // stopLoading();
      } catch (error) {
        // stopLoading();
              ...
              <li>
                ...
                <ul className="sub-menu">
                  {courseCategories.map((category) => (
                    <li key={category.id} className="level1-item">
                      <a
