...
export interface AdminDocumentDTORestoreList {
    id: number;
...
const DocumentManagement: React.FC = () => {
    ...
    const [documents, setDocuments] = useState<AdminDocumentDTORestoreList[]>([]);
    const [filteredDocuments, setFilteredDocuments] = useState<AdminDocumentDTORestoreList[]>([]);
    ...
    const RestoreDocumentFunction = async () => {
        ...
        try {
            const response = await fetch(`${process.env.REACT_APP_SERVER_HOST}/api/general_documents/restore/${selectedDocument}`, {
                method: 'PUT',
                headers: {
                    "Content-Type": "application/json",
            ...
            if (response.ok) {
                const updatedDocument = filteredDocuments.filter(document => document.id !== selectedDocument);
                setDocuments(updatedDocument);
                setFilteredDocuments(updatedDocument);
    ...
    const DeleteDocumentFunction = async () => {
        ...
        try {
            const response = await fetch(`${process.env.REACT_APP_SERVER_HOST}/api/general_documents/delete/${selectedDocument}`, {
                method: 'DELETE',
                headers: {
                    "Content-Type": "application/json",
                    Authorization: `Bearer ${token}`,
    ...
    const handleConfirm = async () => {
        if (action === "restore" && selectedDocument !== null) {
            RestoreDocumentFunction();
        } else if (action === "delete" && selectedDocument !== null) {
    ...
    const fetchAllData = async () => {
        ...
        const url = `${process.env.REACT_APP_SERVER_HOST}/api/general_documents/restore/list-all-documents?${params.toString()}`;
        try {
            ...
            setFilteredDocuments(documentsData.content || []);
    ...
    const handleRestore = (id: number) => {
        ...
        setModalTitle("Khôi phục tài liệu");
        setModalMessage("Bạn có chắc chắn muốn khôi phục tài liệu này?");
        setAction("restore");
        setIsModalOpen(true);
    };
    ...
    const handleDelete = (id: number) => {
        setSelectedDocument(id);
        setModalTitle("Xóa vĩnh viễn tài liệu");
        setModalMessage("Bạn có chắc chắn muốn xóa vĩnh viễn tài liệu này?");
        setAction("delete");
        setIsModalOpen(true);
    };
                ...
                <TableBody>
                    {filteredDocuments.map((document) => (
                        <TableRow key={document.id}>
                            ...
                            <TableCell>{document.deletedDate ? document.deletedDate : "Chưa xóa"}</TableCell>
                            <TableCell>
                                {document.isDeleted ? (
                                    <>
                                        {/* Khôi phục khóa học */}
                                        <IconButton onClick={() => handleRestore(document.id)} color="primary">
                                            <Restore />
                                        </IconButton>
                                        {/* Xóa vĩnh viễn khóa học */}
                                        <IconButton onClick={() => handleDelete(document.id)} color="secondary">
                                            <DeleteForever />
                                        </IconButton>
                                    </>
                                ) : (
