...
export interface AdminTestDTORestoreList {
    id: number;
    createdAt: string; // ISO Date format
    deletedDate: string | null; // Can be null
    description: string;
...
const TestManagement: React.FC = () => {
    const [tests, setTests] = useState<AdminTestDTORestoreList[]>([]);  // Lưu danh sách khóa học
    ...
    const [selectedTest, setSelectedTest] = useState<number | null>(null); // Khóa học đang được chọn
    ...
    const fetchTestList = async () => {
        ...
        try {
            const response = await fetch(
                `${process.env.REACT_APP_SERVER_HOST}/api/tests/restore/list-all-tests?${searchParams.toString()}`,
                {
                    method: "GET",
                    headers: {
                        "Content-Type": "application/json",
    ...
    const RestoreTestFunction = async () => {
        let token = localStorage.getItem("authToken");
        if (isTokenExpired(token)) {
            token = await refresh();
            if (!token) {
        ...
        try {
            const response = await fetch(`${process.env.REACT_APP_SERVER_HOST}/api/tests/restore/${selectedTest}`, {
                method: 'PUT',
                headers: {
                    "Content-Type": "application/json",
                    Authorization: `Bearer ${token}`,
            ...
            if (response.ok) {
                const updatedTests = filteredtests.filter(test => test.id !== selectedTest);
    ...
    const DeleteTestFunction = async () => {
        ...
        try {
            const response = await fetch(`${process.env.REACT_APP_SERVER_HOST}/api/tests/delete/${selectedTest}`, {
                method: 'DELETE',
                headers: {
                    "Content-Type": "application/json",
                    Authorization: `Bearer ${token}`,
    ...
    const handleConfirm = async () => {
        if (action === "restore" && selectedTest !== null) {
            RestoreTestFunction();
        } else if (action === "delete" && selectedTest !== null) {
            DeleteTestFunction();
        }
    };
    ...
    const filterByDate = (chapters: AdminTestDTORestoreList[]) => {
        if (selectedDate) {
            return tests.filter(test => {
                // Chuyển đổi ngày xóa sang đối tượng ngày và so sánh
                const deletedDate = test.deletedDate ? dayjs(test.deletedDate) : null;
    ...
    const filteredtests = filterByDate(
        tests.filter(test =>
    ...
    const getCourseTitle = (courseId: number) => {
        ...
        return course ? course.coursesTitle : "Không xác định";
    };
            ...
            <Table>
                <TableHead>
                    <TableRow>
                        ...
                        <TableCell>Loại bài kiểm tra</TableCell>
                ...
                <TableBody>
                    {filteredtests.map((test) => (
                        <TableRow key={test.id}>
                            ...
                            <TableCell>{getCourseTitle(test.courseId)}</TableCell>
                            <TableCell>{test.isSummary ? "Chương" : "Bài học"}</TableCell>
                            <TableCell>{test.deletedDate ? test.deletedDate : "Chưa xóa"}</TableCell>
                            <TableCell>
                                {test.isDeleted ? (
                                    <>
                                        {/* Khôi phục khóa học */}
                                        <IconButton onClick={() => handleRestore(test.id)} color="primary">
                                            <Restore />
                                        </IconButton>
                                        {/* Xóa vĩnh viễn khóa học */}
                                        <IconButton onClick={() => handleDelete(test.id)} color="secondary">
                                            <DeleteForever />
                                        </IconButton>
                                    </>
                                ) : (
