...
function Profile() {
  ...
  const handleImageChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    if (event.target.files && event.target.files[0]) {
      ...
      setSelectedImagePreview(URL.createObjectURL(file)); // Tạo URL tạm thời để hiển thị ảnh
      setSelectedImage(file); // Lưu file ảnh vào state nếu cần tải lên sau này
    }
  };
            ...
            <div className="col-md-4 text-center">
              <div className="form-group">
                ...
                {selectedImagePreview || profileData?.image ? (
                  <div>
                    <img
                      src={selectedImagePreview || profileData?.image} // Ưu tiên ảnh mới được chọn
                      alt="Ảnh đại diện"
                      style={{
                        width: "150px",
                        height: "150px",
